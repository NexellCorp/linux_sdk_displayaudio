...
static int gem_sync(int drm_fd, int gem_fd, int size)
{
	...
	arg.size = (uint32_t)size;
	arg.flags = 0;
	arg.handle = (uint32_t) gem_fd;

	ret = drm_command_write_read(drm_fd, DRM_NX_GEM_SYNC, &arg,
				     sizeof(arg));
	if (ret) {
		perror("drm_command_write_read\n");
		return ret;
...
#endif //#ifndef USE_ION_ALLOCATOR
