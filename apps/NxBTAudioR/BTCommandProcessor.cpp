...
{
    LOGQ(QString("%1 %2").arg(Q_FUNC_INFO).arg("[START]"));
    char buffer[BUFFER_SIZE];

    if (!m_IPCClient.isRunning())
        m_IPCClient.start((char*)DEFAULT_CLIENT_PATH_FOR_AUDIO, (char*)DEFAULT_SERVER_PATH);

    if (m_IPCClient.isRunning()) {
        emit signalCommandProcessorEnabled();
    }
    ...
    while (m_IPCClient.isRunning()) {
        ...
        LOGQ(QString("BTCommandProcessor [RECV]]: %1").arg(buffer));

        emit signalCommandFromServer(QString("%1").arg(buffer));
    }

    LOGQ(QString("%1 %2").arg(Q_FUNC_INFO).arg("[END]"));
}
